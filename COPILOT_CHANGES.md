...
## 2026-03-17（proof_split 修复）

### 变更任务
- 修复 `proof_split` 被误判为 `LLM output leaked prompt text` 的问题。

### 具体修改内容
- 调整 [src/paper/mdTotex.py](src/paper/mdTotex.py) 的泄漏检测函数：
	- `_has_prompt_leak` 新增参数 `allow_proof_split_markers`。
	- `_validate_llm_tex_output` 新增同名参数并透传。
- 在 `markdown_proof_split_to_latex` 路径中调用：
	- `_validate_llm_tex_output(raw, allow_proof_split_markers=True)`

### 原因说明
- `proof_split` 的正确输出格式本身包含 `<<<PROOF>>>` / `<<<REST>>>`。
- 旧逻辑将这两个标记当作“提示词泄漏”特征，导致误报并触发：
	- `[convert] block N proof-split failed: LLM output leaked prompt text.`
